...
class CopyModelDialog(QtWidgets.QDialog):
    """Pick a source and a target dataset to copy a model between.
    Explicit From/To combo boxes rather than "copy the selected
    dataset's model" plus a target picker, so the direction is never
    ambiguous regardless of what's currently selected in the tree."""

    def __init__(self, datastore, default_source=None, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Copy Model")
        names = datastore.names

        self.source_combo = QtWidgets.QComboBox()
        self.source_combo.addItems(names)
        if default_source in names:
            self.source_combo.setCurrentText(default_source)

        self.target_combo = QtWidgets.QComboBox()
        self.target_combo.addItems(names)
        other = next(
            (n for n in names if n != self.source_combo.currentText()), None
        )
        if other is not None:
            self.target_combo.setCurrentText(other)

        form = QtWidgets.QFormLayout()
        form.addRow("Copy model from:", self.source_combo)
        form.addRow("Copy model to:", self.target_combo)

        hint = QtWidgets.QLabel(
            "This replaces the target's model entirely. Any parameter "
            "elsewhere linked to one of the target's old parameters "
            "will be unlinked."
        )
        hint.setWordWrap(True)
